...
export const icloudAuthService = {
    verifyCredentials: async (email: string, appSpecificPassword: string) => {
        ...
        try {
            ...
            throw new Error('Failed to verify iCloud credentials. Please check your Apple ID and App-Specific Password.');
        }
    },

    getCalendarEvents: async (userId: string) => {
        console.log(`Fetching iCloud events for userId: ${userId}`);
        const stmt = db.prepare('SELECT * FROM calendar_accounts WHERE user_id = ? AND provider = \'icloud\'');
